 ...
 * onboarding preferences (format/discipline) - dedupes to one entry per series so the UI
 * can drill into a specific series' schedule next, via /api/planner/series/:seriesId/sessions.
 *
 ...
 * than silently showing nothing - the response's `tailored` flag tells the frontend
 * which case it's in.
 */
