...
export async function onRequestPost(context: any) {
  ...
  try {
    const summary = await ingestPlannerSubsession(context, subsessionId);
    return json(summary);
  } catch (err: any) {
    if (err instanceof PlannerIngestError) {
    ...
    console.error(JSON.stringify({ level: "error", msg: "planner.subsession.sync.unhandled", message: err?.message ?? String(err) }));
