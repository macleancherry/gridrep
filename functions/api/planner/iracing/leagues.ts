...
import { getLeagueInfo, extractLeagueName, describeIracingError } from "../../../_lib/plannerIracing";
...
export async function onRequestGet(context: any) {
  ...
  const rows = await DB.prepare(
    `SELECT league_id as leagueId, name, last_synced_at as lastSyncedAt, created_at as createdAt,
     ...
     FROM planner_iracing_leagues ORDER BY name ASC`
  ).all<any>();
...
export async function onRequestPost(context: any) {
  ...
  await DB.prepare(
    `INSERT INTO planner_iracing_leagues (league_id, name, last_synced_at, created_at, host_cust_id, session_name_filter)
     VALUES (?, ?, NULL, ?, ?, ?)
