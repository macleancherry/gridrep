...
/**
 * Canonical reasons (these should match src/lib/propReasons.ts)
 */
 ...
 * Map legacy/older reason IDs to canonical ones.
 *
 * Keep this list forever (or until you're comfortable dropping support).
 * It means old clients / old stored UI ids won't break new API validation.
 */
const REASON_ALIASES: Record<string, string> = {
  // Legacy reasons from older backend allowlist:
  respectful_driving: "gave_racing_room",
  great_racecraft: "great_move",
  good_etiquette: "sportsmanship",
  helpful_friendly: "sportsmanship",
  other: "sportsmanship",

  // Common “almost right” values that might exist in the wild:
  clean_race: "clean_battle",
  cleanrace: "clean_battle",
  respectful: "gave_racing_room",
  etiquette: "sportsmanship",
};

function normalizeReason(input: unknown): { raw: string; normalized: string; canonical: string } {
  const raw = String(input ?? "");
  const normalized = raw.trim().toLowerCase();
  const canonical = REASON_ALIASES[normalized] ?? normalized;
...
async function ensureSessionCached(context: any, sessionId: string, accessToken: string) {
  ...
  const exists = await DB.prepare(`SELECT 1 FROM sessions WHERE iracing_session_id = ?`)
    .bind(sessionId)
    .first();
  if (exists) return;
  ...
  await DB.prepare(
    ...
    .bind(sessionId, startTime, seriesName, trackName, split, sof)
    .run();

  const participants = data?.session_results ?? data?.results ?? data?.drivers ?? data?.rows ?? [];
  ...
  if (Array.isArray(participants)) {
    for (const p of participants) {
      ...
      await DB.prepare(
        ...
        .bind(String(memberId), displayName, now)
        .run();
      ...
      await DB.prepare(
        ...
        .bind(sessionId, String(memberId), finishPos, carName)
        .run();
    }
  }
}
...
export async function onRequestPost(context: any) {
  ...
  const { raw, normalized, canonical } = normalizeReason(body.reason);

  if (!CANONICAL_REASONS.has(canonical)) {
  ...
  const giverPart = await DB.prepare(
    ...
    .bind(sessionId, viewer.user!.iracingId)
    .first();
  ...
  try {
    await DB.prepare(
      ...
      .bind(id, sessionId, toDriverId, viewer.user!.id, canonical, createdAt)
      .run();
  } catch {
