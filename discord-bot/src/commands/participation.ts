...
export const participationCommand: CommandHandler = async (interaction, env) => {
  ...
  if (sessions.length === 0) {
    return { embeds: [{ title: `${series.seriesName} — participation`, description: "No scheduled sessions found.", color: BRAND_COLOR }] };
  }

  const { matrix, xLabels, yLabels } = buildDayHourMatrix(sessions, (s) => s.field_size ?? 0);
  const chartUrl = buildHeatmap(xLabels, yLabels, matrix, `${series.seriesName} — driver density by day/hour (UTC)`);

  return {
    embeds: [
      { title: `${series.seriesName} — participation density`, color: BRAND_COLOR, image: { url: chartUrl } },
    ],
  };
};
